...
from server.oldsrc.core.settings import Tags, GAMES_ROUTER_PREFIX, GAMES_ASSETS_PATH, \
...
from server.oldsrc.api.v1.endpoints.assets import router as assets_router
from server.oldsrc.api.v1.endpoints.game_tags import router as tags_router
from server.oldsrc.api.v1.endpoints.reviews import router as reviews_router
...
@router.get('/')
async def items(title: str = Query(None),
    ...
    if title:
        games_query = games_query.filter(Game.title.ilike(f'%{title}%'))
    ...
    if current_user is None or current_user.is_superuser:
        games_query = games_query.filter(Game.is_published == True)
    else:
